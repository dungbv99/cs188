...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        if len(self.keys()) == 0:
            return None
        all = list(self.items())
        distribution = [i[1] for i in all]
        values = [i[0] for i in all]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        #ghostPosition = gameState.getGhostPosition(0)
        s = 0
        for pos in self.allPositions:
            #print(pos,self.beliefs[pos])
            self.beliefs[pos] *= self.getObservationProb(observation,pacmanPosition,pos,jailPosition)
            #print(self.getObservationProb(observation,pacmanPosition,pos,jailPosition),self.beliefs[pos])
            #print(pacmanPosition,jailPosition,pos)
            s += self.beliefs[pos]
        #print(self.beliefs)
        if s != 0:
            self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        all_possible = DiscreteDistribution()
        #print(all_possible)
        for pos in self.allPositions:
            new_distribution = self.getPositionDistribution(gameState, pos)
            #print(pos,"*",new_distribution)
            #print(self.beliefs[pos])
            for new_postion, probability in new_distribution.items():
                
                all_possible[new_postion] = all_possible[new_postion] + probability *self.beliefs[pos] 
