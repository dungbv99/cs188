...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        minGhostDistance = -1
        for probGhost in livingGhostPositionDistributions:
            maxValue = -1
            pos = None
            for state, prob in probGhost.items():
                if maxValue < prob:
                    maxValue = prob
                    pos = state
            distance = self.distancer.getDistance(pos,pacmanPosition)
            if closestGhostPos == None or distance <  minGhostDistance:
        ...
        for action in legal:
            ...
            if minAction == None or distance > dist:
                distance = dist
                minAction = action
        return minAction
